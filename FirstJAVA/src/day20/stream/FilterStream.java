...
public class FilterStream {
	...
	public static void main(String[] args) {
		...
		// "홍"으로 시작하는 문자열로 필터링 후 내부 반복자를 이용하여 출력
		System.out.println("filter() - '홍'으로 시작하는 것 필터링");
		list.stream().filter(n -> n.startsWith("홍")).forEach(n -> System.out.print(n+" ")); //predicate로 true인 것만 forEach로 출력
		System.out.println();
		
		//중복 제거 후에 "홍"으로 시작하는 문자열을 내부 반복자를 이용하여 출력
