...
public class FileExample {
	...
	public static void main(String[] args) {
		...
		//절대 경로 : 파일이나 디렉터리를 지정할 때 루트(최상위경로)부터 모든 경로 정보를 다 기술하는 경우 
		//			>어디서든 같은 경로를 가지고 있습니다.
		//			>단, 절대 경로는 경로의 길이 값이 길어질 수 있음. 이 때문에 불편할 수 있음
		//상대 경로 : 파일이나 디렉터리를 지정할 때 현재 작업 위치를 기준으로 경로 정보를 기술하는 경우
		//			>작업 위치에 따라서 변경이 됩니다.
		//			>절대 경로와 달리 상대적인 경로이기 때문에 간단하게 경로를 기술하는 것이 가능
		
		System.out.println("isAbsolute? "+f.isAbsolute());
		System.out.println("isAbsolute? "+dir.isAbsolute());
		
		//4. canExecute, canRead, canWrite : 권한 확인		
		System.out.println("canExecute? "+f.canExecute());	//실행 권환
		...
		System.out.println("canWrite? "+f.canWrite());	//쓰기 권한
		System.out.println("canWrite? "+dir.canWrite());
		
		//5. getAbsolutePath : 절대 경로 반환(상대 경로의 절대 경로를 찾아줌)
		...
		//mkdirs() : 존재하지 않은 부모 폴더까지 모두 포함해서 폴더를 생성
		if(!test.exists()) {
