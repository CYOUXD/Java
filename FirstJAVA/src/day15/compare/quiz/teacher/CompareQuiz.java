package day15.compare.quiz.teacher; //7
...
public class CompareQuiz {
	...
	public static void main(String[] args) {
		...
		TreeSet<GymMember> gym2 = new TreeSet<GymMember>(new Comparator<GymMember>() {		//내부클래스 중 익명 클래스 사용
			@Override
			public int compare(GymMember o1, GymMember o2) {
				//## compare 메서드 사용법
				//- o1과 o2의 자리를 바꾸고 싶은 경우 1을 리턴
				//- o1과 o2의 자리를 그대로 유지하고 싶은 경우 -1을 리턴
				//- o1과 o1의 값이 같은 경우 0을 리턴
				//이렇게 안하고 compareTo를 사용하면 더 잘 나옴
				if(o1.name.charAt(0) < o2.name.charAt(0)) {	//charAt(0) - 첫번째 글자만 비교
					return -1;
...
class GymMember implements Comparable<GymMember> {
	...
	public int compareTo(GymMember o) {
		// CompareTo() 메서드 사용법(매개변수로 비교 대상을 전달)
