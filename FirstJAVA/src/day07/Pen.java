...
public class Pen {
	...
	/*
	 * ** 생성자는 반드시 클래스 안에 하나 이상 언언해야 합니다.
	 * 	만약 선언하지 않으면 JNM(JAVA Virtual Machine)이 대신해서 기본생성자를 자동으로 만들어줌!
	 * ** 기본 생성자는 매개변수를 가지지 않은 형태로, 자동으로 만들어지는 경우 내용은 없음.
	 * 
	 * ** 생성자를 만드는 규칙 **
	 * 	1. 생성자의 이름은 클래스 이름과 대소문자까지 정확히 같아야 합니다.	클래스 이름과 다른 이름으로 만들면 그것은 메서드 취급!
	...
	//생성자
	public Pen() {	//기본 생성자
		color = "black";	//멤버 변수에는 초기화 하고 싶은 값만 넣으면 된다.
	}
	
	//멤버 초기화 생성자
	public Pen(String init_color) {
		color = init_color; 	//매개변수명이 멤버변수명과 같지 않아서 중복되지 않으므로 this를 안써도 됨.
